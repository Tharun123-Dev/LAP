// Topbar.jsx — Fully Responsive
// Mobile: hamburger menu + compact notification bell + avatar
// Tablet/Desktop: page title + date + notification bell + avatar
...
const TYPE_ICONS = {
  ...
  attendance_absent: '🔴', regularization:    '🔄',
  payroll_processed: '💰', leave_balance_low: '⚠️',
  new_account:       '👋', general:           '🔔',
  policy_updated:    '⚙️',
}
const TYPE_ROUTE = {
  leave_applied:     '/dashboard/leave',       leave_approved:    '/dashboard/leave',
  leave_rejected:    '/dashboard/leave',       leave_cancelled:   '/dashboard/leave',
  attendance_absent: '/dashboard/attendance',  regularization:    '/dashboard/attendance',
  payroll_processed: '/dashboard/payroll',     leave_balance_low: '/dashboard/leave',
  new_account:       '/dashboard/employees',   policy_updated:    '/dashboard/settings/system',
...
const getPageTitle = (role, permissions, pathname) => {
  const allItems = (role === 'superadmin' || role === 'admin')
    ? SUPERADMIN_NAV
    : NAV_ITEMS.filter(item =>
        item.always || (item.codes && item.codes.some(c => permissions.includes(c)))
      )
...
export default function Topbar({ onMenuClick, isMobile }) {
  ...
  const handleMarkAndNavigate = async (notif, e) => {
    ...
    setOpen(false)
    navigate(TYPE_ROUTE[notif.type] || '/dashboard/notifications')
  }
  ...
  // Notification dropdown width: full on mobile, 380px on desktop
  const dropW = isMobile ? 'calc(100vw - 24px)' : '380px'
  const dropRight = isMobile ? '-8px' : '0'
    ...
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          {!isMobile && (
            <p style={{ fontSize: '11px', color: '#aaa', margin: 0 }}>{now}</p>
          )}
        </div>
      </div>
            ...
            <div style={{
              ...
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  {unread > 0 && (
                    <button onClick={markAll} style={linkBtn('#4338ca')}>Mark all read</button>
                  )}
                  <button onClick={() => { setOpen(false); navigate('/dashboard/notifications') }} style={linkBtn('#6b7280')}>
                    See all
                  </button>
                </div>
              </div>
