...
//  3. Fully responsive: mobile (≤640px), tablet (641–1024px), desktop (>1024px)
// ─────────────────────────────────────────────────────────────────────────────
...
// ── SVG Mini-Map ──────────────────────────────────────────────────────────────
function MiniMap({ gps, office, compact }) {
  ...
  const scale = radiusPx / office.radius_meters
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span>🔴 Office</span>
        <span style={{ color: '#6366f1' }}>◌ Zone ({office.radius_meters} m)</span>
...
export default function TodayWidget() {
  ...
  // Layout: desktop = side by side, tablet/mobile = stacked
  const useColumnLayout = !isMobile && !isTablet && showMap
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        gridTemplateColumns: useColumnLayout ? '1fr 420px' : undefined,
        ...
        <div style={{ minWidth: 0, width: '100%', boxSizing: 'border-box' }}>
          ...
          <div style={{
            ...
            <InfoCard
              ...
              sub={today?.is_wfh ? 'Work From Home' : (today?.checkin_distance_m != null ? `📍 ${Math.round(today.checkin_distance_m)} m` : 'Office')}
            ...
            <InfoCard
              ...
              sub={today?.check_out
                ? `${today.hours_worked}h${today.checkout_distance_m != null ? ` · 📍 ${Math.round(today.checkout_distance_m)} m` : ''}`
                : 'Not yet'}
          ...
          {/* Map — shown inline on mobile/tablet (below controls, before button) */}
          {showMap && (isMobile || isTablet) && (
            <div style={{ marginBottom: '14px' }}>
              <MiniMap gps={gps} office={office} compact={isMobile} />
              {!locLoading ? (
                <button onClick={fetchGps} style={refreshBtnStyle}>
                  🔄 Refresh My Location
                </button>
              ) : (
        ...
        {useColumnLayout && (
          <div style={{ position: 'sticky', top: '16px', width: '100%', boxSizing: 'border-box' }}>
            <MiniMap gps={gps} office={office} compact={false} />
            {!locLoading ? (
              <button onClick={fetchGps} style={refreshBtnStyle}>
                🔄 Refresh My Location
              </button>
...
function InfoCard({ icon, label, value, sub, highlight, compact }) {
  return (
    <div style={{
      ...
      <p style={{
        margin:    compact ? '6px 0 3px' : '8px 0 4px',
        fontSize:  compact ? '18px' : '24px',
        fontWeight: 700,
        color:     highlight ? '#166534' : '#aaa',
        overflow:  'hidden',
